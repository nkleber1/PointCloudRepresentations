...
class PointNetSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        """
        B = Batch (a batch is a point cloud)
        C = Coordinates (position of the point)
        N = N Points (in the point cloud)
        Input:
...
class PointNet2Encoder(nn.Module):
    def __init__(self, args):
        ...
        self.sa3 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=320 + 2,
                                          mlp=[128, 256, 1024], group_all=True)
